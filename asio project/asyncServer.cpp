...
public:
    ...
    void do_continuous_broadcast() {
        while (true) {
            // Broadcast the message to all clients
            std::string received_message = messageMap.printAll();
            ws_.write(boost::asio::buffer(received_message));
    ...
    void on_accept(beast::error_code ec) {
        ...
        do_read();
        do_continuous_broadcast();
    }

    void do_read() {
        // Read a message into our buffer
        ws_.async_read( buffer_, beast::bind_front_handler( &session::on_read, shared_from_this()));
        std::cout << boost::beast::buffers_to_string(buffer_.cdata()) << std::endl;
    }
